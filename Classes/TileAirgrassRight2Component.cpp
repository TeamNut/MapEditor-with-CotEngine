...
#include "MapEditor.h"
#define THIS_TILE "tile_airgrass_right_02.png"
#define RADIUS_BLOCK 25 //Area of half of block

TileAirgrassRight2Component* TileAirgrassRight2Component::Init(MapEditor* lpScene)
{
	lpMapEditor = lpScene;
	myPos = 50;
...
void TileAirgrassRight2Component::Update(Time& time)
{
	//Position Control
	if (IsKeyStay(KeyCode::Comma)) myPos--;
	if (IsKeyStay(KeyCode::Period)) myPos++;
	GetOwner()->SetLocalPosition(Vec3(myPos + GAP_OF_BLOCK * 10, RESOLUTION_Y - 85));
