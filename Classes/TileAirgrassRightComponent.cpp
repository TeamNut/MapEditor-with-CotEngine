...
#include "MapEditor.h"
#define THIS_TILE "tile_airgrass_right_01.png"
#define RADIUS_BLOCK 25 //Area of half of block

TileAirgrassRightComponent* TileAirgrassRightComponent::Init(MapEditor* lpScene)
{
	lpMapEditor = lpScene;
	myPos = 50;
...
void TileAirgrassRightComponent::Update(Time& time)
{
	//Position Control
	if (IsKeyStay(KeyCode::Comma)) myPos--;
	if (IsKeyStay(KeyCode::Period)) myPos++;
	GetOwner()->SetLocalPosition(Vec3(myPos + GAP_OF_BLOCK * 9, RESOLUTION_Y - 85));
